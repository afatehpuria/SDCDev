...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        if abs(distance) < 0.0 :
            raise ValueError, "Car cannot move backwards"
        ...
        gauss_steering = random.gauss(steering,steering_noise)
        gauss_distance = random.gauss(distance,distance_noise)
        #result.set_noise(self.bearing_noise, self.steering_noise, self.distance_noise)

        beta = (gauss_distance/length)*tan(gauss_steering)
        if abs(beta) < 0.001:
            new_x           = self.x + gauss_distance*cos(self.orientation)
            new_y           = self.y + gauss_distance*sin(self.orientation)
            new_orientation = (self.orientation + beta)%(2.0*pi)
        else:
            R               = gauss_distance/beta
    ...
    def sense(self, option=1): 
        ...
        if option == 0:
            ...
            for i in range(4):
                ...
                dy              = (landmarks[i][0] - self.y)
                dx              = (landmarks[i][1] - self.x)
                bearing         = (atan2(dy,dx) - self.orientation)%(2.0*pi)
                gauss_bearing   = random.gauss(bearing, self.bearing_noise)
                Z.append(gauss_bearing)
...
## function check_output (see test case 2). Note: Even for a well-
## implemented particle filter this function occasionally returns
## False. This is because a particle filter is a randomized 
## algorithm. We will be testing your code multiple times. Make 
## sure check_output returns True at least 80% of the time.
